...
namespace ks114_sonar
{
    ...
    static const std::map<int, uint8_t> SONAR_ADDRESSES {
    ...
    static const std::map<uint8_t, std::string_view> NOISE_SUPPRESSIONS {
    ...
    static const std::map<uint8_t, std::string_view> BEAM_ANGLES {{0X7A, "MODE_0"},
                                                           {0X7B, "MODE_1_DEFAULT"},
                                                           {0X7C, "MODE_2"},
                                                           {0X7D, "MODE_3"},
                                                           {0X7E, "MODE_4"},
    ...
    static constexpr double DATA_GIVEN_IF_ERROR {404.0};
    class Ks114Sonar
    {
        public:
        // Ks114Sonar() = delete;
        ...
        ~Ks114Sonar();
        std::vector<uint8_t> getSenseCommand(uint8_t address, DetectionMode mode);
        bool start();
        bool stop();
