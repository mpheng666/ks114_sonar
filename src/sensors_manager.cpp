...
SensorsManager::SensorsManager(ros::NodeHandle &nh)
    : nh_p_(nh),
      ...
      get_data_stimer_(nh_p_.createSteadyTimer(ros::WallDuration(0.05), &SensorsManager::timerGetDataSteadyCallBack, this)),
...
void SensorsManager::loadParams()
{
    ...
    if (!nh_p_.param("sonar_remapper", sonar_remapper_,
                     sonar_remapper_))
    {
        for (const auto &remapper : sonar_remapper_)
        {
            ROS_WARN_STREAM("sonar_remapper is not set! Use default "
                            << remapper);
        }
    }
}
...
void SensorsManager::updateVariables()
{
    ...
    sonar_remapper_.resize(num_of_sensor_);
}
