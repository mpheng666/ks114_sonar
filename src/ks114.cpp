...
namespace ks114_ns
{
    ...
    void SonarKs114::initSensorsConnectionState(std::map<int, bool>& states, const int size)
    {
    ...
    bool SonarKs114::updateSensorConnectionState(std::map<int, bool>& states, const int index, const bool state)
    {
            ...
            [&](const auto& address)
            {
                ...
                if(ser_.available())
                {
                    ...
                    if(byte_received.at(5) == address)
                    {
                        ...
                        this->updateSensorConnectionState(sonars_connection_state_, index, true);
                    }
                }
                else
                {
                    ROS_ERROR("Sonar %d is NOT CONNECTED OR CONFIGURED!", index);
                    this->updateSensorConnectionState(sonars_connection_state_, index, false);
        ...
        std::for_each(
            ...
            [&](const auto& address)
            {
                ...
                if(ser_.available())
                {
                    ...
                    {
                        #ifdef DEBUG
                            ROS_INFO("disconnected_sensor size: %li", disconnected_sensors_.size());
                        #endif
                        this->updateSensorConnectionState(sonars_connection_state_, index, true);
