#include "sam_mkitchen_temperature_sensor/comms_handler.hpp"

namespace sam_mk {
    CommsHandler::CommsHandler()
        : serial_port_(ioc_)
        , connection_handler_cb_timer_(
          ioc_, std::chrono::duration(std::chrono::milliseconds(connection_timer_ms_)))
    {
        setupSerialPort();
    }
                             ...
                             unsigned int expected_return_duration_ms = 0)
    {
        if (current_connection_state_ == ConnectionState::CONNECTED) {
            ...
            [[maybe_unused]]auto write_byte = serial_port_.write_some(boost::asio::buffer(data), ec);
            if (ec) {
    ...
    std::vector<uint8_t> CommsHandler::getReadBuffer()
    {
        ...
        const std::vector<uint8_t> retval{read_buff_byte_.begin(), read_buff_byte_.end()};
    ...
    void CommsHandler::connectSerial()
    {
        if (!serial_port_.is_open()) {
            try {
                ...
                std::cerr << "Connection error with port " << port_name_ << " " << e.what() << '\n';
                current_connection_state_ = ConnectionState::ERROR;
            }
        }
        else {
    ...
    void CommsHandler::asyncRead([[maybe_unused]] const boost::system::error_code& ec)
    {
        if (current_connection_state_ == ConnectionState::CONNECTED) {
            ...
            boost::bind(&CommsHandler::handleRead, this, boost::asio::placeholders::error,
    ...
    [maybe_unused]] const boost::system::error_code& ec)
    {
        if (target_connection_state_ != current_connection_state_) {
            ...
            // static_cast<int>(current_connection_state_)
            //           << "\n";
        }
